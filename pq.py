...
class PQ(object):
    ...
    def fit(self, vecs, iter):
        ...
        # [q + 1, ..., q + 1, q, ..., q]
        dims_width = [quotient + 1 if i <
                      reminder else quotient for i in range(self.M)]
        ...
        self.codewords = np.zeros(
            (self.M, self.Ks, np.max(self.Ds)), dtype=np.float32)
        for m in range(self.M):
            if self.verbose:
                print("#    Training the subspace: {} / {}, {} -> {}".format(m,
                                                                             self.M, self.Ds[m], self.Ds[m+1]))
    ...
    def decode(self, codes):
        ...
        for m in range(self.M):
            vecs[:, self.Ds[m]: self.Ds[m+1]] = self.codewords[m,
                                                               codes[:, m], :self.Ds[m+1] - self.Ds[m]]
...
class MPQ(object):
    ...
    def decode(self, codes):
        ...
        vecs = np.empty(
            (self.numTable, N, self.tables[0].Dim), dtype=np.float32)
        for i in range(self.numTable):
